...
$del_mat1 = "DELETE FROM `orcamento_material` WHERE orcamento_id = '$id'";
$del_mat = mysqli_query($conn, $del_mat1)or die( mysqli_error($conn));
...
/*
$add = "UPDATE orcamento (data_orca, funcionario_id, cliente_nome, mao_obra, tipo_orca, observacao, valor_total, estoque, status, count)
VALUES ('$data_orca', '$funcionario_id', '$cliente_id', '$mao_obra', '$tipo', 'teste', '$cria', '', '$status', '0') WHERE id_orcamnto = '$id'";
$add_orca = mysqli_query($conn, $add)or die( mysqli_error($conn));
$id = mysqli_insert_id($conn);//pega o id_orcamnto do que foi adicionado
*/
...
if ($assento_tecido != '') {
  ...
  @$val_tot_tecido_ace = round((($val['SUM(preco_total)'] / $qto_tecido_ass['quantidade']) * $tecido_ass), 2);// faz a conta e mostra so dois digitos depois da virgula ou ponto

   if (($qto_tecido_ass['quantidade'] - $tecido_ass) < 0) {
     $obs_estoc_tec_ass = "{$qto_tecido_ass['quantidade']}  {$qto_tecido_ass['uni_medida']} DE {$qto_tecido_ass['nome_mat']} NO ESTOQUE, E O ORÇAMENTO É DE  {$tecido_ass}  {$qto_tecido_ass['uni_medida']} DE {$qto_tecido_ass['nome_mat']}";
     $count = +1;
 }else {
  $obs_estoc_tec_ass ='';
 }
  ...
  VALUES ('$id', '$assento_tecido', '$tecido_ass', '$val_tot_tecido_ace')";
...
if ($assento_enchimento != '') {
  ...
  @$val_tot_enchimento_ace = round((($val['SUM(preco_total)'] / $qto['quantidade']) * $espuma_ass) * $assento, 2);// faz a conta e mostra so dois digitos depois da virgula ou ponto

   if (($qto['quantidade'] - $espuma_ass) < 0) {
     $obs_estoc_esp_ass = "{$qto['quantidade']}  {$qto['uni_medida']} DE {$qto['nome_mat']} NO ESTOQUE, E O ORÇAMENTO É DE  {$espuma_ass}  {$qto['uni_medida']} DE {$qto['nome_mat']}";
     $count = +1;
 }else {
  $obs_estoc_esp_ass ='';
 }
  ...
  $add_orca_mat_prep = "INSERT INTO orcamento_material (orcamento_id, material_id, quantidade_mat, total_mat)
  VALUES ('$id', '$assento_enchimento', '$espuma_ass', '$val_tot_enchimento_ace')";
  $add_orca_mat = mysqli_query($conn, $add_orca_mat_prep)or die( mysqli_error($conn));
...
if ($assento_ziper1 != '') {
 ...
 $obs_estoc_zip_ass ='';
  ...
  $add_orca_mat_prep = "INSERT INTO orcamento_material (orcamento_id, material_id, quantidade_mat, total_mat)
  VALUES ('$id', '$assento_ziper', '$conv_lar_ace1', '$val_tot_ziper_ace')";
  $add_orca_mat = mysqli_query($conn, $add_orca_mat_prep)or die( mysqli_error($conn));
  ...
  $val_tot_ziper_ace = 0;
    ...
    @$val_tot_botao_ace = round((($val['SUM(preco_total)'] / $qto['quantidade']) * $assento_botao_qto) * $assento, 2);
    $assento_botao_qto1 = $assento_botao_qto * $assento;

    if (($qto['quantidade'] - $assento_botao_qto1) < 0) {
      $obs_estoc_bot_ass = "{$qto['quantidade']}  {$qto['uni_medida']} DE {$qto['nome_mat']} NO ESTOQUE, E O ORÇAMENTO É DE  {$assento_botao_qto1}  {$qto['uni_medida']} DE {$qto['nome_mat']}";
      $count = +1;
  }else {
   $obs_estoc_bot_ass ='';
  }
    ...
    $add_orca_mat_prep = "INSERT INTO orcamento_material (orcamento_id, material_id, quantidade_mat, total_mat)
    VALUES ('$id', '$assento_botao', '$assento_botao_qto1', '$val_tot_botao_ace')";
    $add_orca_mat = mysqli_query($conn, $add_orca_mat_prep)or die( mysqli_error($conn));
...
if ($encosto_tecido != '') {
  ...
  @$val_tot_tecido_enc = round((($val['SUM(preco_total)'] / $qto['quantidade']) * $tecido_enc), 2);// faz a conta e mostra so dois digitos depois da virgula ou ponto

  if (($qto['quantidade'] - $tecido_enc) < 0) {
    $obs_estoc_tec_enc = "{$qto['quantidade']}  {$qto['uni_medida']} DE {$qto['nome_mat']} NO ESTOQUE, E O ORÇAMENTO É DE  {$tecido_enc}  {$qto['uni_medida']} DE {$qto['nome_mat']}";
    $count = +1;
}else {
 $obs_estoc_tec_enc ='';
}
  ...
  $add_orca_mat_prep = "INSERT INTO orcamento_material (orcamento_id, material_id, quantidade_mat, total_mat)
  VALUES ('$id', '$encosto_tecido', '$tecido_enc', '$val_tot_tecido_enc')";
  $add_orca_mat = mysqli_query($conn, $add_orca_mat_prep)or die( mysqli_error($conn));
...
if ($encosto_enchimento1 != '') {
  ...
  @$val_tot_enchimento_enc = round((($val['SUM(preco_total)'] / $qto['quantidade']) * $espuma_enc) * $encosto, 2);// faz a conta e mostra so dois digitos depois da virgula ou ponto

  if (($qto['quantidade'] - $espuma_enc) < 0) {
    $obs_estoc_esp_enc = "{$qto['quantidade']}  {$qto['uni_medida']} DE {$qto['nome_mat']} NO ESTOQUE, E O ORÇAMENTO É DE  {$espuma_enc}  {$qto['uni_medida']} DE {$qto['nome_mat']}";
    $count = +1;
}else {
 $obs_estoc_esp_enc ='';
}
  ...
  $add_orca_mat_prep = "INSERT INTO orcamento_material (orcamento_id, material_id, quantidade_mat, total_mat)
  VALUES ('$id', '$encosto_enchimento', '$espuma_enc', '$val_tot_enchimento_enc')";
  $add_orca_mat = mysqli_query($conn, $add_orca_mat_prep)or die( mysqli_error($conn));
...
if ($encosto_ziper1 != '') {
  ...
  $val_tot_ziper_enc = round(((($val['SUM(preco_total)'] / $qto['quantidade']) * $conv_lar_enc) * $encosto), 2);// faz a conta e mostra so dois digitos depois da virgula ou ponto
  $conv_lar_enc1 = $conv_lar_enc * $encosto;

  if (($qto['quantidade'] - $conv_lar_enc1) < 0) {
    $obs_estoc_zip_enc = "{$qto['quantidade']}  {$qto['uni_medida']} DE {$qto['nome_mat']} NO ESTOQUE, E O ORÇAMENTO É DE  {$conv_lar_enc1}  {$qto['uni_medida']} DE {$qto['nome_mat']}";
}else {
 $obs_estoc_zip_enc ='';
}

  $add_orca_mat_prep = "INSERT INTO orcamento_material (orcamento_id, material_id, quantidade_mat, total_mat)
  VALUES ('$id', '$encosto_ziper', '$conv_lar_enc1', '$val_tot_ziper_enc')";
  $add_orca_mat = mysqli_query($conn, $add_orca_mat_prep)or die( mysqli_error($conn));
  ...
  $val_tot_ziper_enc = 0;
    ...
    @$val_tot_botao_enc = round((($val['SUM(preco_total)'] / $qto['quantidade']) * $encosto_botao_qto) * $encosto, 2);
    $encosto_botao_qto1 = $encosto_botao_qto * $encosto;

    if (($qto['quantidade'] - $encosto_botao_qto1) < 0) {
      $obs_estoc_bot_enc = "{$qto['quantidade']}  {$qto['uni_medida']} DE {$qto['nome_mat']} NO ESTOQUE, E O ORÇAMENTO É DE  {$encosto_botao_qto1}  {$qto['uni_medida']} DE {$qto['nome_mat']}";
      $count = +1;
  }else {
   $obs_estoc_bot_enc ='';
  }
    ...
    $add_orca_mat_prep = "INSERT INTO orcamento_material (orcamento_id, material_id, quantidade_mat, total_mat)
    VALUES ('$id', '$encosto_botao', '$encosto_botao_qto1', '$val_tot_botao_enc')";
    $add_orca_mat = mysqli_query($conn, $add_orca_mat_prep)or die( mysqli_error($conn));
...
$almofada_tecido1 = $_POST['tecido_almofada'];
if ($almofada_tecido1 != '') {
  ...
  if ($margem_alm == "") {
    $metro_quadrado_alm = $conv_lar_ace * $conv_pro_ace;  //mertro quadrado sem margem de seguraça
    $tecido_alm = $metro_quadrado_alm * $almofada;
  }else {
    $metro_quadrado_alm = (($conv_lar_ace * $conv_pro_ace) + $margem_alm);   //mertro quadrado
  ...
  @$val_tot_tecido_alm = round((($val['SUM(preco_total)'] / $qto['quantidade']) * $tecido_alm), 2);// faz a conta e mostra so dois digitos depois da virgula ou ponto

  if (($qto['quantidade'] - $tecido_alm) < 0) {
    $obs_estoc_tec_alm = "{$qto['quantidade']}  {$qto['uni_medida']} DE {$qto['nome_mat']} NO ESTOQUE, E O ORÇAMENTO É DE  {$tecido_alm}  {$qto['uni_medida']} DE {$qto['nome_mat']}";
    $count = +1;
}else {
 $obs_estoc_tec_alm ='';
}
  ...
  $add_orca_mat_prep = "INSERT INTO orcamento_material (orcamento_id, material_id, quantidade_mat, total_mat)
  VALUES ('$id', '$almofada_tecido', '$tecido_alm', '$val_tot_tecido_alm')";
  $add_orca_mat = mysqli_query($conn, $add_orca_mat_prep)or die( mysqli_error($conn));
...
if ($almofada_enchimento1 != '') {
  ...
  @$val_tot_enchimento_alm = round((($val['SUM(preco_total)'] / $qto['quantidade']) * $espuma_alm) * $almofada, 2);// faz a conta e mostra so dois digitos depois da virgula ou ponto

  if (($qto['quantidade'] - $espuma_alm) < 0) {
    $obs_estoc_esp_alm = "{$qto['quantidade']}  {$qto['uni_medida']} DE {$qto['nome_mat']} NO ESTOQUE, E O ORÇAMENTO É DE  {$espuma_alm}  {$qto['uni_medida']} DE {$qto['nome_mat']}";
    $count = +1;
}else {
 $obs_estoc_esp_alm ='';
}
  ...
  $add_orca_mat_prep = "INSERT INTO orcamento_material (orcamento_id, material_id, quantidade_mat, total_mat)
  VALUES ('$id', '$almofada_enchimento', '$espuma_alm', '$val_tot_enchimento_alm')";
  $add_orca_mat = mysqli_query($conn, $add_orca_mat_prep)or die( mysqli_error($conn));
...
if ($almofada_ziper1 != '') {
  ...
  @$val_tot_ziper_alm = round(((($val['SUM(preco_total)'] / $qto['quantidade']) * $conv_lar_alm) * $almofada), 2);// faz a conta e mostra so dois digitos depois da virgula ou ponto
  $conv_lar_alm1 = $conv_lar_alm * $almofada;

  if (($qto['quantidade'] - $conv_lar_alm1) < 0) {
    $obs_estoc_zip_alm = "{$qto['quantidade']}  {$qto['uni_medida']} DE {$qto['nome_mat']} NO ESTOQUE, E O ORÇAMENTO É DE  {$conv_lar_alm1}  {$qto['uni_medida']} DE {$qto['nome_mat']}";
    $count = +1;
}else {
 $obs_estoc_zip_alm ='';
}
  ...
  $add_orca_mat_prep = "INSERT INTO orcamento_material (orcamento_id, material_id, quantidade_mat, total_mat)
  VALUES ('$id', '$almofada_ziper', '$conv_lar_alm1', '$val_tot_ziper_alm')";
  $add_orca_mat = mysqli_query($conn, $add_orca_mat_prep)or die( mysqli_error($conn));
  ...
  $val_tot_ziper_alm = 0;
    ...
    @$val_tot_botao_alm = round((($val['SUM(preco_total)'] / $qto['quantidade']) * $almofada_botao_qto) * $almofada, 2);
    $almofada_botao_qto1 = $almofada_botao_qto * $almofada;

    if (($qto['quantidade'] - $almofada_botao_qto1) < 0) {
      $obs_estoc_bot_alm = "{$qto['quantidade']}  {$qto['uni_medida']} DE {$qto['nome_mat']} NO ESTOQUE, E O ORÇAMENTO É DE {$almofada_botao_qto1}  {$qto['uni_medida']} DE {$qto['nome_mat']}";
      $count = +1;
  }else {
   $obs_estoc_bot_alm ='';
  }
    ...
    $add_orca_mat_prep = "INSERT INTO orcamento_material (orcamento_id, material_id, quantidade_mat, total_mat)
    VALUES ('$id', '$almofada_botao', '$almofada_botao_qto1', '$val_tot_botao_alm')";
    $add_orca_mat = mysqli_query($conn, $add_orca_mat_prep)or die( mysqli_error($conn));
...
if ($estrutura1 != '') {
  ...
  @$val_tot_est = round(($val['SUM(preco_total)'] / $qto['quantidade']) * $estrutura_comprimento, 2);

  if (($qto['quantidade'] - $estrutura_comprimento) < 0) {
    $obs_estoc_est = "{$qto['quantidade']}  {$qto['uni_medida']} DE {$qto['nome_mat']} NO ESTOQUE, E O ORÇAMENTO É DE {$estrutura_comprimento}  {$qto['uni_medida']} DE {$qto['nome_mat']}";
    $count = +1;
}else {
 $obs_estoc_est ='';
}
  ...
  $add_orca_mat_prep = "INSERT INTO orcamento_material (orcamento_id, material_id, quantidade_mat, total_mat)
  VALUES ('$id', '$estrutura', '$estrutura_comprimento', '$val_tot_est')";
  $add_orca_mat = mysqli_query($conn, $add_orca_mat_prep)or die( mysqli_error($conn));
...
$add_update = "UPDATE orcamento SET data_orca = '$data_orca', funcionario_id = '$funcionario_id', cliente_nome = '$cliente_id', mao_obra = '$mao_obra', tipo_orca = '$tipo', observacao = '$observacao', valor_total = '$valor_total', estoque = '$observacao_estoque', status = '$status', count = '0' WHERE id_orcamento = '$id' ";
...
$result_orcamentos = "SELECT * FROM orcamento
  ...
  INNER JOIN material ON nome_mat = (SELECT nome_mat FROM material WHERE id_material = material_id) WHERE id_orcamento = '$id'";
$resultado_orcamentos = mysqli_query($conn, $result_orcamentos);
$orcamento = mysqli_fetch_assoc($resultado_orcamentos);

$orc_mat1 = "SELECT * FROM orcamento_material WHERE orcamento_id = '$id'";
$resul_orc_mat = mysqli_query($conn, $orc_mat1);

$tot_mat1 = "SELECT SUM(total_mat) FROM `orcamento_material` WHERE orcamento_id = '$id'";
...
*/
header('Location: ../../orcamentos.php?pesquisa=&pagina=1#');
 ?>
